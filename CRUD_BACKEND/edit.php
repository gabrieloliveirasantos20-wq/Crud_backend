...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($erros)) {
        $stmt = $pdo->prepare("UPDATE produtos SET
            ...
            quantidade=:quantidade, preco=:preco, data_cadastro=:data_cadastro
            WHERE id=:id");
        $stmt->execute([
            ':nome' => $dados['nome'],
        ...
        $_SESSION['flash'] = ['type' => 'success', 'msg' => 'Produto atualizado com sucesso!'];
    ...
    $dados = $stmt->fetch();
    if (!$dados) {
        $_SESSION['flash'] = ['type' => 'error', 'msg' => 'Produto não encontrado.'];
        header('Location: index.php');
        exit;
    }
}
